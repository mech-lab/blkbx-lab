...
def _scenario_happy_path() -> dict[str, Any]:
    ...
    exported_bundle = bundle.export(
        ...
        verification_reports=[{"status": "passed", "report_ref": "verify/happy-path-report.json"}],
    )
    exported_bundle = copy.deepcopy(exported_bundle)
    exported_bundle["generated_at"] = "2026-07-17T09:07:00Z"
    exported_bundle = refresh_integrity(exported_bundle)
    return {
        "workspace": {
...
def _scenario_human_review_edge() -> dict[str, Any]:
    ...
    exported_bundle = bundle.export(
        ...
        authority_receipts=[authority_receipt],
        verification_reports=[{"status": "passed", "report_ref": "verify/human-review-edge-report.json"}],
    )
    exported_bundle = copy.deepcopy(exported_bundle)
    exported_bundle["generated_at"] = "2026-07-17T10:06:00Z"
    exported_bundle = refresh_integrity(exported_bundle)
    return {
        "workspace": {
...
def _scenario_incident_to_renewal() -> dict[str, Any]:
    ...
    exported_bundle = copy.deepcopy(exported_bundle)
    exported_bundle["generated_at"] = "2026-07-17T11:06:00Z"
    exported_bundle = refresh_integrity(exported_bundle)
    return {
        "workspace": {
...
def available_scenarios() -> list[str]:
    return ["lloyds_incident_to_renewal"]
