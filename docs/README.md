# Docs Hub

This hub tracks the shipped BLKBX Lab documentation surface as of July 17, 2026. It is organized by audience so onboarding, reference, operator guidance, and historical material stay separate.

## Getting Started

- [Repository landing page](../README.md)
- [Install and package surface](pypi.md)
- [CLI and API contract](mvp-cli-api-contract.md)
- [Public object spec](public-object-spec.md)

## CLI and API Reference

- [CLI and API contract](mvp-cli-api-contract.md)
- [Public object spec](public-object-spec.md)
- [Adapter contract](adapter-contract.md)

## Product Slices

- [Product architecture](product-architecture.md)
- [BLKBXS bank-verifiable receipts](bank-verifiable-receipts.md)
- [MAND8 source-slice README](../products/mand8-sdk/README.md)
- [DUE source-slice README](../products/due-sdk/README.md)

## Architecture

- [Developer architecture](developer-architecture.md)
- [Product architecture](product-architecture.md)
- [Receipt domain model](receipt-domain-model.md)
- [Formal semantics draft](../FORMAL_SEMANTICS.md)

## Verification and Security

- [Independent verification](independent-verification.md)
- [Technical security hardening note](security-hardening.md)
- [Public vulnerability policy](../SECURITY.md)
- [Reproducibility guide](../REPRODUCIBILITY.md)
- [Qwen3.5 validation report](research/qwen35-validation-report.md)

## Release and Contributor Operations

- [Contributing guide](../CONTRIBUTING.md)
- [Release operator guide](../RELEASING.md)
- [Release readiness checklist](release-readiness.md)
- [Brand architecture](brand-architecture.md)
- [Brand system](brand/README.md)

## Migration and Compatibility

- [Migration and compatibility notes](migration-compatibility.md)
- [Migration from `mech-lab`](migration-from-mech-lab.md)

## Archive

- [Archive index](archive/README.md)

Archived documents remain in-repo for historical context, but they are not part of the primary onboarding or release-facing contract.
