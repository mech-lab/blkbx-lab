...
def test_python_versions_are_synced_for_v07_release():
    root_project = _toml(ROOT / "pyproject.toml")

    assert root_project["project"]["version"] == "0.7.0"
    assert __version__ == "0.7.0"


def test_workspace_crates_share_the_v07_release_version():
    workspace = _toml(ROOT / "Cargo.toml")
    assert workspace["workspace"]["package"]["version"] == "0.7.0"
    cargo_paths = (
